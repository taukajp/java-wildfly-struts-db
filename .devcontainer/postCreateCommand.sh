...
! grep -q -E 'oracle' ${CONFIG_FILE} \
  && sed -i -e '/driver name="h2"/e cat .devcontainer/oracle/driver.part' -e '/jndi-name="java:jboss\/datasources\/ExampleDS"/e cat .devcontainer/oracle/datasource.part' ${CONFIG_FILE}
...
test ! -e ${SERVERS_DIR}/wildfly-${WILDFLY_VER}/modules/system/layers/base/com/oracle/main/ojdbc11-${OR_DRIVER_VER}.jar \
  && mvn dependency:get -DgroupId=com.oracle.database.jdbc -DartifactId=ojdbc11 -Dversion=${OR_DRIVER_VER} \
  && cp /workspaces/myapp/.m2/repository/com/oracle/database/jdbc/ojdbc11/${OR_DRIVER_VER}/ojdbc11-${OR_DRIVER_VER}.jar ${SERVERS_DIR}/wildfly-${WILDFLY_VER}/modules/system/layers/base/com/oracle/main
